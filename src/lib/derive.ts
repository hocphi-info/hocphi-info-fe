...
import type { MajorRow, SchoolStats } from "@/types/domain";
...
/**
 * Thống kê học phí năm đầu cho các ngành HỆ ĐẠI TRÀ của một trường.
 * `rows` là các MajorRow đã lọc sẵn về một trường (mọi track); hàm này tự bỏ
 * các track khác đại trà — không bao giờ trộn hệ vào một phép min/median/max.
 */
export function schoolTuitionStats(rows: MajorRow[]): SchoolStats {
  const daiTra = rows.filter((r) => r.program.track === "dai_tra");

  const byAmount = [...daiTra].sort(
  ...
  const increasePcts = daiTra
    .map((r) => r.increase?.annualIncreasePct)
    .filter((p): p is number => p != null);
  const increaseSources = new Set(
    daiTra.map((r) => r.increase?.increaseSource).filter(Boolean),
  );
